...
*
*   Why is this needed?
*   - In Goodmorningnetwork some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::FillBloc(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward){
    ...
    if(nBlockHeight >=1 && nBlockHeight <= 3) return;
    CAmount amout = GetBlockSubsidy(0,nBlockHeight,Params().GetConsensus(),false) ;
    txNew.vout[0].nValue = txNew.vout[0].nValue - amout * 0.3;

    CScript FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress(DEV_FUNDER_ADDRESS).Get());
    txNew.vout.push_back(CTxOut(amout*0.15, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));

    CScript FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress(GAME_FUNDER_ADDRESS).Get());
    txNew.vout.push_back(CTxOut(amout*0.15, CScript(FOUNDER_2_SCRIPT.begin(), FOUNDER_2_SCRIPT.end())));
}
...
{
    if(fLiteMode) return; // disable all Goodmorningnetwork specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f GM\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
