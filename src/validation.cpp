...
#if defined(NDEBUG)
# error "Goodmorningnetwork Core cannot be compiled without assertions."
#endif
...
//    LogPrintf("----------------halvings= %i ----------------\n", halvings);
    if(nHeight < 10  || (halvings > (consensusParams.nSubsidyHalvingInterval - 20)
        && halvings < (consensusParams.nSubsidyHalvingInterval + 20)))
    {
        return true;
    }
    bool found_1 = false;
    ...
    CScript FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress(DEV_FUNDER_ADDRESS).Get());
    CScript FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress(GAME_FUNDER_ADDRESS).Get());

    CAmount amount = GetBlockSubsidy(0,nHeight,consensusParams,false);

//    LogPrint("----------------addr= %s,amount=nValue=%d.%08d ----------------\n",
//             DEV_FUNDER_ADDRESS,amount / COIN * 0.15, amount  % COIN * 0.15);
//    LogPrint("----------------addr= %s,amount=nValue=%d.%08d ----------------\n",
//             GAME_FUNDER_ADDRESS,amount / COIN* 0.2 , amount % COIN* 0.2 );
//
//    LogPrintf("----------------CheckFoundersInputs:tx.GetHash=%s\n tx=%s\n", tx.GetHash().ToString(),tx.ToString());
    BOOST_FOREACH(const CTxOut &output, tx.vout)
    {
        ...
        if (output.scriptPubKey == FOUNDER_1_SCRIPT && output.nValue == amount * 0.15)
        {
            found_1 = true;
            continue;
        }

        if (output.scriptPubKey == FOUNDER_2_SCRIPT && output.nValue == amount * 0.15)
        {
            found_2 = true;
            continue;
        }
...
{
    CAmount nSubsidy = blockValue * 0.2; // start at 20%
    return 0;
}
...
void ThreadScriptCheck() {
    RenameThread("Goodmorningnetwork-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // GM : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GM): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(GM): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // GM : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GM): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GM): spork is off, skipping transaction locking checks\n");
    }
