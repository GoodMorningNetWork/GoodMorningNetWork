...
// ***TODO*** ScanHash is not yet used in Goodmorningnetwork
//
...
{
    LogPrintf("GoodmorningnetworkMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Goodmorningnetwork-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("GoodmorningnetworkMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("GoodmorningnetworkMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("GoodmorningnetworkMiner:\n  proof-of-work found\n  hash: %s\n  target: %s\n", hash.GetHex(), hashTarget.GetHex());
    ...
    {
        LogPrintf("GoodmorningnetworkMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("GoodmorningnetworkMiner -- runtime error: %s\n", e.what());
        return;
    }
}
