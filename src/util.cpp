...
//Goodmorningnetwork only features
...
const char * const BITCOIN_CONF_FILENAME = "Goodmorningnetwork.conf";
const char * const BITCOIN_PID_FILENAME = "Goodmorningnetworkd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "Goodmorningnetwork" is a composite category enabling all Goodmorningnetwork-related debug output
            if(ptrCategory->count(string("Goodmorningnetwork"))) {
...
#else
    const char* pszModule = "Goodmorningnetwork";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GoodmorningnetworkCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GoodmorningnetworkCore
    // Mac: ~/Library/Application Support/GoodmorningnetworkCore
    // Unix: ~/.Goodmorningnetworkcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GoodmorningnetworkCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GoodmorningnetworkCore";
#else
    // Unix
    return pathRet / ".Goodmorningnetworkcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Goodmorningnetwork.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override Goodmorningnetwork.conf
