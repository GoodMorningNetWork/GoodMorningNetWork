...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Goodmorningnetwork Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Goodmorningnetwork Core is downloading blocks...");
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Goodmorningnetwork Core is downloading masternode winners...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Goodmorningnetwork Core is syncing with network...");
    ...
    CAmount amount = GetBlockSubsidy(0,pindexPrev->nHeight,consensusParams,false);
    CBitcoinAddress dev_funder(DEV_FUNDER_ADDRESS);
    CBitcoinAddress game_funder(GAME_FUNDER_ADDRESS);

    CScript DEV_FOUNDER_SCRIPT = GetScriptForDestination(dev_funder.Get());
    CScript GAME_FOUNDER_SCRIPT = GetScriptForDestination(dev_funder.Get());

    UniValue entry1(UniValue::VOBJ);
    entry1.push_back(Pair("payee", dev_funder.ToString().c_str()));
    entry1.push_back(Pair("script", HexStr(DEV_FOUNDER_SCRIPT.begin(), DEV_FOUNDER_SCRIPT.end())));
    entry1.push_back(Pair("amount", amount*0.15));
    foundationArray.push_back(entry1);


    UniValue entry2(UniValue::VOBJ);
    entry2.push_back(Pair("payee", game_funder.ToString().c_str()));
    entry2.push_back(Pair("script", HexStr(GAME_FOUNDER_SCRIPT.begin(), GAME_FOUNDER_SCRIPT.end())));
    entry2.push_back(Pair("amount", amount*0.15));
    foundationArray.push_back(entry2);
